...
def test_public_metrics_summary_keeps_resume_metrics_honest():
    ...
    assert payload["public_metrics"]["test_count"] == 73
    ...
    assert payload["verified_project_outcomes"]["tool_allowlist_count"] == 6
    ...
    assert payload["verified_project_outcomes"]["implemented_agent_capabilities"] == 15
    ...
    assert "6 allowed agent tools and 3 rejected unsafe PostgreSQL queries" in payload["resume_safe_signals"]
