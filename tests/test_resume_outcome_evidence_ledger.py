...
def test_resume_outcome_evidence_ledger_separates_claimable_and_blocked_outcomes():
    ...
    assert payload["blocked_until_evidence_count"] == 6
    ...
    assert payload["public_counts"]["accepted_real_model_runs"] == 0
    ...
    assert any(item["metric"] == "accepted_real_model_runs" for item in payload["blocked_until_evidence"])
