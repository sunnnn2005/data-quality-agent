...
def test_live_project_scorecard_summarizes_public_resume_evidence_without_inflation():
    ...
    assert payload["headline_metrics"]["passing_tests"] == 70
    assert payload["headline_metrics"]["verified_resume_claims"] == 30
    assert payload["headline_metrics"]["implemented_agent_capabilities"] == 14
