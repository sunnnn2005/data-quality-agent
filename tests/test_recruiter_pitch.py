...
def test_recruiter_pitch_turns_verified_evidence_into_safe_application_language():
    ...
    assert verification["evidence_link_count"] == 23
    ...
    assert any(link["label"] == "Inspect real-model evidence capture" for link in payload["evidence_links"])
    ...
    assert any(link["label"] == "Use reviewer evidence kit" for link in payload["evidence_links"])
