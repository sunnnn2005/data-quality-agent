...
def test_application_evidence_pack_gives_recruiters_verified_review_path():
    ...
    assert len(payload["application_links"]) == 43
    ...
    assert payload["verified_outcome_numbers"]["passing_tests"] == 149
    assert payload["verified_outcome_numbers"]["verified_resume_claims"] == 83
    ...
    assert payload["verified_outcome_numbers"]["outcome_collection_evidence_fields"] == 23
