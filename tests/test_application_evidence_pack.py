...
def test_application_evidence_pack_gives_recruiters_verified_review_path():
    ...
    assert payload["verified_outcome_numbers"]["passing_tests"] == 90
    assert payload["verified_outcome_numbers"]["verified_resume_claims"] == 49
    ...
    assert payload["verified_outcome_numbers"]["public_traction_growth_channels"] == 14
