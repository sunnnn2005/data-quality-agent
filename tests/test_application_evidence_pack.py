...
def test_application_evidence_pack_gives_recruiters_verified_review_path():
    ...
    assert payload["verified_outcome_numbers"]["passing_tests"] == 239
    ...
    assert payload["verified_outcome_numbers"]["public_traction_growth_channels"] == 21
    assert payload["verified_outcome_numbers"]["feedback_intake_required_sections"] == 7
    ...
    assert payload["verified_outcome_numbers"]["resume_outcome_scoreboard_claimable_now"] == 6
