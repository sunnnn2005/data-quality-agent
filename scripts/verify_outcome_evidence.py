...
def verify_manifest() -> dict[str, int]:
    ...
    for claim in claims:
        ...
        if metric_name:
            if metric_name == "external_feedback_items":
                ...
                if public_metrics_summary.get("public_metrics", {}).get("test_count") != 235:
                ...
                if application_pack.get("verified_outcome_numbers", {}).get("passing_tests") != 235:
    ...
    if badges_by_id.get("ci-tests", {}).get("message") != "235 passing":
    ...
    if launch_evidence_snapshot.get("application_pack", {}).get("passing_tests") != 235:
    ...
    if "<strong>235</strong><span>automated tests passing locally and in ci</span>" not in index_page:
