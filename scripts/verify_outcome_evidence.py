...
def verify_manifest() -> dict[str, int]:
    ...
    for claim in claims:
        ...
        if metric_name:
            if metric_name == "external_feedback_items":
                ...
                if public_metrics_summary.get("public_metrics", {}).get("test_count") != 164:
                ...
                if application_pack.get("verified_outcome_numbers", {}).get("passing_tests") != 164:
    ...
    if "public-metrics-summary" in claim_ids:
        ...
        for phrase in ("passing ci tests | 164", "confirmed external users | 0", "forks | 1"):
    ...
    if "<strong>164</strong><span>automated tests passing locally and in ci</span>" not in index_page:
