...
LLM_VALUE_COMPARISON_PATH = ROOT / "docs" / "llm-value-comparison.json"
...
LAUNCH_EVIDENCE_SNAPSHOT_PATH = ROOT / "docs" / "launch-evidence-snapshot.json"
...
PRIVATE_REVIEWER_LEAD_WORKFLOW_PATH = ROOT / "docs" / "private-reviewer-lead-workflow.json"
...
def verify_manifest() -> dict[str, int]:
    ...
    public_metrics_provenance = load_payload(PUBLIC_METRICS_PROVENANCE_PATH)
    agent_readiness = load_payload(AGENT_READINESS_PATH)
    eval_summary = load_payload(EVAL_SUMMARY_PATH)
    llm_value_comparison = load_payload(LLM_VALUE_COMPARISON_PATH)
    ...
    launch_evidence_snapshot = load_payload(LAUNCH_EVIDENCE_SNAPSHOT_PATH)
    ...
    private_reviewer_lead_workflow = load_payload(PRIVATE_REVIEWER_LEAD_WORKFLOW_PATH)
    ...
    for claim in claims:
        ...
        if metric_name:
            if metric_name == "external_feedback_items":
                ...
                if public_metrics_summary.get("public_metrics", {}).get("test_count") != 243:
                ...
                if public_metrics_summary.get("public_metrics", {}).get("claimable_public_metrics") != 2:
                    raise AssertionError("public metrics summary must include provenance claimable metric count")
            ...
            elif metric_name == "resume_outcome_evidence_ledger":
                ledger_script = (ROOT / "scripts" / "build_resume_outcome_evidence_ledger.py").read_text()
                ledger_tests = (ROOT / "tests" / "test_resume_outcome_evidence_ledger.py").read_text()
                ledger_page = (ROOT / "docs" / "resume-outcome-evidence-ledger.md").read_text()
                expected = {
                ...
                for key, value in expected.items():
                    if resume_outcome_evidence_ledger.get(key) != value:
                        raise AssertionError(f"resume outcome evidence ledger {key} expected {value!r}")
                for key in (
                    "confirmed_external_users",
                    "external_feedback_items",
                    "ai_engineer_review_items",
                ...
                if resume_outcome_evidence_ledger.get("public_counts", {}).get("stars") != 0:
                    raise AssertionError("resume outcome evidence ledger must preserve zero stars")
                for required in ("record_reviewer_outreach_event.py", "build_accepted_evidence_rollup.py"):
                    if required not in ledger_script:
                        raise AssertionError(f"resume outcome evidence ledger script missing {required}")
                for required in ("Resume Outcome Evidence Ledger", "Blocked Until Public Evidence", "Not Claimed"):
                    if required not in ledger_page:
                        raise AssertionError(f"resume outcome evidence ledger page missing {required}")
                if "test_resume_outcome_evidence_ledger_separates_claimable_and_blocked_outcomes" not in ledger_tests:
                    raise AssertionError("resume outcome evidence ledger must include a dedicated test")
                if claim.get("metric_value") != 1:
                    raise AssertionError("resume_outcome_evidence_ledger claim must use metric_value=1")
            elif metric_name == "pilot_reviewer_crm":
                crm_script = (ROOT / "scripts" / "build_pilot_reviewer_crm.py").read_text()
                ...
                for key, value in expected.items():
                    if pilot_reviewer_crm.get(key) != value:
                        raise AssertionError(f"pilot reviewer CRM {key} expected {value!r}")
                if pilot_reviewer_crm.get("leads", [])[0].get("target_metric") != "ai_engineer_review_items":
                    raise AssertionError("pilot reviewer CRM must prioritize AI Engineer review first")
                for required in (
                    "ai_engineer_review_items",
                    "confirmed_external_users",
                    "reproducible_feedback_items",
                    ...
                    if required not in pilot_reviewer_crm.get("target_counts", {}):
                        raise AssertionError(f"pilot reviewer CRM missing target metric {required}")
                for required in ("record_reviewer_outreach_event.py", "Do not buy", "enterprise deployment"):
                    if required not in json.dumps(pilot_reviewer_crm, sort_keys=True):
                        raise AssertionError(f"pilot reviewer CRM missing boundary {required}")
                for required in ("Pilot Reviewer CRM", "Three-Week Sprint", "Operating Rules"):
                    if required not in crm_page:
                        raise AssertionError(f"pilot reviewer CRM page missing {required}")
                if "verify_pilot_reviewer_crm" not in crm_script:
                    raise AssertionError("pilot reviewer CRM must include a script verifier")
                if "test_pilot_reviewer_crm_turns_outcomes_into_executable_leads" not in crm_tests:
                    raise AssertionError("pilot reviewer CRM must include a dedicated test")
                if claim.get("metric_value") != 1:
                    raise AssertionError("pilot_reviewer_crm claim must use metric_value=1")
            elif metric_name == "private_reviewer_lead_workflow":
                lead_script = (ROOT / "scripts" / "build_private_reviewer_lead_workflow.py").read_text()
                lead_tests = (ROOT / "tests" / "test_private_reviewer_lead_workflow.py").read_text()
                ...
                if private_reviewer_lead_workflow.get("private_paths_gitignored") is not True:
                    raise AssertionError("private reviewer lead workflow must verify gitignored private paths")
                if "private/" not in gitignore or "private-reviewer-leads.csv" not in gitignore:
                    raise AssertionError("private reviewer lead paths must remain in .gitignore")
                if private_reviewer_lead_workflow.get("required_columns") != expected_columns:
                    raise AssertionError("private reviewer lead workflow must expose the expected CSV columns")
                if private_reviewer_lead_workflow.get("required_column_count") != 11:
                    raise AssertionError("private reviewer lead workflow must expose 11 required columns")
                if private_reviewer_lead_workflow.get("allowed_status_count") != 6:
                    raise AssertionError("private reviewer lead workflow must expose six statuses")
                if private_reviewer_lead_workflow.get("target_metric_count") != 6:
                    raise AssertionError("private reviewer lead workflow must expose six target metrics")
                joined = json.dumps(private_reviewer_lead_workflow, sort_keys=True).lower()
                for required in (
                    ...
                    if required not in joined:
                        raise AssertionError(f"private reviewer lead workflow missing {required}")
                if "verify_private_reviewer_lead_workflow" not in lead_script:
                    raise AssertionError("private reviewer lead workflow script must include verifier")
                if "test_private_reviewer_lead_workflow_keeps_contacts_out_of_public_repo" not in lead_tests:
                    raise AssertionError("private reviewer lead workflow tests must cover privacy boundaries")
                if claim.get("metric_value") != 1:
                    raise AssertionError("private_reviewer_lead_workflow claim must use metric_value=1")
            elif metric_name == "private_reviewer_lead_summary":
                summary_script = (ROOT / "scripts" / "build_private_reviewer_lead_summary.py").read_text()
                summary_tests = (ROOT / "tests" / "test_private_reviewer_lead_summary.py").read_text()
                if private_reviewer_lead_summary.get("resume_outcome_upgraded") is not False:
                    raise AssertionError("private reviewer lead summary must not upgrade resume outcomes")
                if private_reviewer_lead_summary.get("sensitive_columns_excluded") != [
                    "notes_private",
                    "private_contact_label",
                ]:
                    raise AssertionError("private reviewer lead summary must exclude sensitive columns")
                if private_reviewer_lead_summary.get("private_source_path") != "private/reviewer-leads.csv":
                    raise AssertionError("private reviewer lead summary must read from the ignored local CSV path")
                if private_reviewer_lead_summary.get("accepted_ready_count", 0) > private_reviewer_lead_summary.get(
                    "public_evidence_url_count", 0
                ):
                    raise AssertionError("private reviewer lead summary cannot have more accepted-ready rows than URLs")
                joined = json.dumps(private_reviewer_lead_summary, sort_keys=True).lower()
                for required in (
                    "private lead rows are not public evidence",
                    ...
                    if required not in joined:
                        raise AssertionError(f"private reviewer lead summary missing {required}")
                if "verify_private_reviewer_lead_summary" not in summary_script:
                    raise AssertionError("private reviewer lead summary script must include verifier")
                for required_test in (
                    "test_private_reviewer_lead_summary_validates_redacted_progress",
                    "test_private_reviewer_lead_summary_reports_schema_and_gate_errors",
                ):
                    if required_test not in summary_tests:
                        raise AssertionError(f"private reviewer lead summary missing test {required_test}")
                if claim.get("metric_value") != 1:
                    raise AssertionError("private_reviewer_lead_summary claim must use metric_value=1")
            elif metric_name == "outcome_witness_packet":
                witness_script = (ROOT / "scripts" / "build_outcome_witness_packet.py").read_text()
                witness_tests = (ROOT / "tests" / "test_outcome_witness_packet.py").read_text()
                if outcome_witness_packet.get("resume_outcome_upgraded") is not False:
                    raise AssertionError("outcome witness packet must not upgrade resume outcomes")
                expected = {
                ...
                for key, value in expected.items():
                    if outcome_witness_packet.get(key) != value:
                        raise AssertionError(f"outcome witness packet {key} expected {value!r}")
                if any(value != 0 for value in outcome_witness_packet.get("current_public_counts", {}).values()):
                    raise AssertionError("outcome witness packet must preserve zero external outcome counts")
                joined = json.dumps(outcome_witness_packet, sort_keys=True)
                for required in (
                    ...
                    if required not in joined:
                        raise AssertionError(f"outcome witness packet missing {required}")
                if "verify_outcome_witness_packet" not in witness_script:
                    raise AssertionError("outcome witness packet script must include verifier")
                for required_test in (
                    "test_outcome_witness_packet_turns_reviewers_into_countable_public_tasks",
                    "test_outcome_witness_packet_rejects_unverified_resume_upgrade",
                ):
                    if required_test not in witness_tests:
                        raise AssertionError(f"outcome witness packet missing test {required_test}")
                if claim.get("metric_value") != 1:
                    raise AssertionError("outcome_witness_packet claim must use metric_value=1")
            elif metric_name == "outcome_sprint_plan":
                sprint_script = (ROOT / "scripts" / "build_outcome_sprint_plan.py").read_text()
                sprint_tests = (ROOT / "tests" / "test_outcome_sprint_plan.py").read_text()
                expected = {
                    "sprint_day_count": 5,
                    "target_metric_count": 5,
                ...
                for key, value in expected.items():
                    if outcome_sprint_plan.get(key) != value:
                        raise AssertionError(f"outcome sprint plan {key} expected {value!r}")
                if any(value != 0 for value in outcome_sprint_plan.get("current_public_counts", {}).values()):
                    raise AssertionError("outcome sprint plan must preserve zero public outcome counts")
                joined = json.dumps(outcome_sprint_plan, sort_keys=True).lower()
                for required in (
                    ...
                    if required not in joined:
                        raise AssertionError(f"outcome sprint plan missing {required}")
                if "verify_outcome_sprint_plan" not in sprint_script:
                    raise AssertionError("outcome sprint plan script must include verifier")
                for required_test in (
                    "test_outcome_sprint_plan_turns_zero_outcomes_into_real_next_actions",
                    "test_outcome_sprint_plan_does_not_fabricate_resume_outcomes",
                ):
                    if required_test not in sprint_tests:
                ...
                for key, value in expected.items():
                    if resume_claim_materializer.get(key) != value:
                        raise AssertionError(
                            f"resume_claim_materializer {key} mismatch: "
                            f"{resume_claim_materializer.get(key)} != {value}"
                        )
                required_metrics = {
                    "ai_engineer_review_items",
                    "accepted_real_model_runs",
                ...
                materializer_metrics = {
                    item.get("metric") for item in resume_claim_materializer.get("future_bullet_templates", [])
                }
                if materializer_metrics != required_metrics:
                    raise AssertionError("resume_claim_materializer future template metrics mismatch")
                if any(item.get("materialized") for item in resume_claim_materializer.get("future_bullet_templates", [])):
                    raise AssertionError("resume_claim_materializer must not materialize zero-count outcome bullets")
                for required in [
                    "verify_resume_claim_materializer",
                    "FUTURE_BULLET_TEMPLATES",
                    "exact resume bullets",
                ]:
                    if required not in materializer_script:
                        raise AssertionError(f"resume claim materializer script missing {required}")
                for required in [
                    "test_resume_claim_materializer_blocks_external_outcome_bullets_without_evidence",
                    "test_resume_claim_materializer_markdown_is_recruiter_readable",
                ]:
                    if required not in materializer_tests:
                        raise AssertionError(f"resume claim materializer tests missing {required}")
                for required in ["Resume Claim Materializer", "Materialized Outcome Bullets", "Blocked Future Templates"]:
                    if required not in materializer_page:
            ...
            elif metric_name == "adaptive_strategy_recall_lift":
                comparison_tests = (ROOT / "tests" / "test_llm_value_comparison.py").read_text()
                comparison_script = (ROOT / "scripts" / "build_llm_value_comparison.py").read_text()
                if llm_value_comparison.get("scenario_count") != 14:
                    raise AssertionError("LLM value comparison must cover the 14-scenario eval set")
                if llm_value_comparison.get("fixed_generic_average_recall") != 0.417:
                    raise AssertionError("LLM value comparison must preserve the fixed baseline recall")
                if llm_value_comparison.get("adaptive_strategy_average_recall") != 1.0:
                    raise AssertionError("LLM value comparison must preserve adaptive strategy recall")
                if llm_value_comparison.get("absolute_recall_lift") != claim.get("metric_value"):
                    raise AssertionError(
                        f"claim {claim['id']} metric mismatch: adaptive_strategy_recall_lift="
                        f"{claim.get('metric_value')} but comparison has "
                        f"{llm_value_comparison.get('absolute_recall_lift')}"
                    )
                if llm_value_comparison.get("improved_scenario_count", 0) < 8:
                    raise AssertionError("LLM value comparison must improve a majority of scenarios")
                if "verify_llm_value_comparison" not in comparison_script:
                    raise AssertionError("LLM value comparison script must verify generated metrics")
                if "test_llm_value_comparison_quantifies_adaptive_strategy_lift" not in comparison_tests:
                    raise AssertionError("LLM value comparison must have a dedicated test")
                ...
                if application_pack.get("verified_outcome_numbers", {}).get("passing_tests") != 243:
            ...
            elif metric_name == "resume_outcome_adjudication":
                adjudication_script = (ROOT / "scripts" / "build_resume_outcome_adjudication.py").read_text()
                adjudication_tests = (ROOT / "tests" / "test_resume_outcome_adjudication.py").read_text()
                if adjudication.get("claim_category_count") != 5:
                    raise AssertionError("resume outcome adjudication must verify 5 outcome categories")
                if adjudication.get("claimable_category_count") != 0:
                    raise AssertionError("resume outcome adjudication must keep claimable external outcomes at zero")
                if adjudication.get("blocked_category_count") != 5:
                    raise AssertionError("resume outcome adjudication must verify 5 blocked categories")
                if adjudication.get("accepted_issue_count") != 0:
                    raise AssertionError("resume outcome adjudication must preserve zero accepted issue count")
                for required in ("external users", "customer feedback", "business validation"):
                    if required not in adjudication.get("not_claimed", []):
                        raise AssertionError(f"resume outcome adjudication must not claim {required}")
                if "verify_resume_outcome_adjudication" not in adjudication_script:
                    raise AssertionError("resume outcome adjudication must include a script verifier")
                if "test_resume_outcome_adjudication_keeps_strong_resume_claims_evidence_gated" not in adjudication_tests:
                    raise AssertionError("resume outcome adjudication must include a dedicated test")
                if claim.get("metric_value") != 1:
                    raise AssertionError("resume_outcome_adjudication claim must use metric_value=1")
    ...
    if "public-metrics-refresh" in claim_ids:
        ...
        for required in (
            ...
            "scripts/build_github_public_stats_snapshot.py",
            ...
            "scripts/build_public_metrics_provenance.py",
    ...
    provenance_metrics = {item["metric"]: item for item in public_metrics_provenance.get("metrics", [])}
    if public_metrics_provenance.get("metric_count") != 8:
        raise AssertionError("public metrics provenance must track eight outcome metrics")
    if public_metrics_provenance.get("claimable_metric_count") != 2:
        raise AssertionError("public metrics provenance must keep only two metrics claimable")
    if public_metrics_provenance.get("blocked_or_baseline_metric_count") != 5:
        raise AssertionError("public metrics provenance must preserve five blocked or baseline metrics")
    if provenance_metrics.get("github_stars", {}).get("resume_status") != "baseline_only":
        raise AssertionError("zero GitHub stars must remain baseline-only in public metrics provenance")
    if provenance_metrics.get("github_forks", {}).get("resume_status") != "claimable":
        raise AssertionError("public fork count must remain claimable in public metrics provenance")
    if provenance_metrics.get("passing_tests", {}).get("value") != metrics.get("test_count"):
        raise AssertionError("public metrics provenance passing test count must match adoption metrics")
    for key in (
        "confirmed_external_users",
        "external_feedback_items",
        "business_case_feedback_items",
        ...
        item = provenance_metrics.get(key, {})
        if item.get("value") != 0:
            raise AssertionError(f"{key} must remain zero in public metrics provenance")
        if item.get("resume_status") != "blocked_until_accepted_evidence":
            raise AssertionError(f"{key} must stay blocked until accepted evidence exists")
        if item.get("counts_match_gate") is not True:
            raise AssertionError(f"{key} must match the accepted evidence gate")
    for phrase in ("github public api fallback", "accepted evidence-gate counts", "self-authored planning issues"):
        if phrase not in " ".join(public_metrics_provenance.get("source_controls", [])).lower():
            raise AssertionError(f"public metrics provenance missing source-control phrase: {phrase}")

    public_stats_page = (ROOT / "docs" / "github-public-stats-snapshot.md").read_text().lower()
    public_stats = github_public_stats.get("public_stats", {})
    if github_public_stats.get("repo") != "https://github.com/sunnnn2005/data-quality-agent":
        raise AssertionError("GitHub public stats snapshot must point to the project repository")
    for key in ("stars", "forks", "watchers", "subscribers", "open_issues"):
        if not isinstance(public_stats.get(key), int) or public_stats[key] < 0:
            raise AssertionError(f"GitHub public stats snapshot must provide non-negative {key}")
    if public_stats.get("stars") != metrics.get("stars"):
        raise AssertionError("GitHub public stats stars must match adoption metrics stars")
    if public_stats.get("forks") != metrics.get("forks"):
        raise AssertionError("GitHub public stats forks must match adoption metrics forks")
    for phrase in ("stars | 0", "confirmed users", "customer feedback", "stars above the live public count"):
        if phrase not in public_stats_page:
            raise AssertionError(f"GitHub public stats page missing phrase: {phrase}")
    ...
    if badges_by_id.get("ci-tests", {}).get("message") != "243 passing":
    ...
    if launch_evidence_snapshot.get("launch_surface_count") != 5:
    ...
    if launch_evidence_snapshot.get("application_pack", {}).get("passing_tests") != 243:
    ...
    if launch_evidence_snapshot.get("review_path", {}).get("current_count") != 0:
        raise AssertionError("launch evidence snapshot must preserve zero accepted AI Engineer reviews")
    if launch_evidence_snapshot.get("claimable_now_count") != 5:
        raise AssertionError("launch evidence snapshot must expose five claimable current launch signals")
    if launch_evidence_snapshot.get("blocked_claim_count") != 5:
    ...
    if "<strong>243</strong><span>automated tests passing locally and in ci</span>" not in index_page:
    ...
    if "launch-evidence-snapshot.md" not in index_page or "launch evidence snapshot" not in index_page:
        raise AssertionError("public homepage must link recruiters to the launch evidence snapshot")
